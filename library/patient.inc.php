...
function genFacilityTitle($repname = '', $facid = 0, $logo = "")
{
    ...
    if (empty($logo)) {
        ...
        $s .= "  <td align='center' class='ftitlecellm'><h2>   Clinical Note</h2></td>\n"; 
//     $s .= "  <td align='left' class='ftitlecellm'><h2>" . text($repname) . "</h2></td>\n";
    }
    $s .= "  <td align='right' class='ftitlecell2'>\n";
    $r = getFacility($facid);
    if (!empty($r)) { 
        $s .= "<b>" . htmlspecialchars($r['name'] ?? '', ENT_QUOTES, 'UTF-8') . "</b>\n"; 
        if (!empty($r['street'])) { 
            $s .= "<br />" . htmlspecialchars($r['street'], ENT_QUOTES, 'UTF-8') . "\n"; 
       }
...
//    $s .= 'Treatment Note'          //text($patient_name) . '&emsp;DOB: ' . text($patient_dob['DOB_TS']);
//    if ($DOS) {
//        $s .= '&emsp;DOS: ' . text($DOS);
//    }
