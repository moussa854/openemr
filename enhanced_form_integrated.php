...
if ($form_id) {
    ...
    error_log("=== DEBUG LOADING: Loading secondary medications for form_id: $form_id");
    error_log("=== DEBUG LOADING: SQL query: $sql");
    ...
    while ($row = sqlFetchArray($result)) {
        ...
        error_log("=== DEBUG LOADING: Found secondary medication #" . $row['medication_order'] . ": " . json_encode($row));
    }
    
    error_log("=== DEBUG LOADING: Total secondary medications found: $count");
    error_log("=== DEBUG LOADING: Existing secondary medications array: " . json_encode($existing_secondary_medications));
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <form method="POST" action="save_enhanced.php" id="enhanced-infusion-form">
                    ...
                    <div class="form-section">
                        <h3 class="section-title"><i class="fa fa-prescription"></i> Medication Order</h3>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="date" name="order_expiration_date" id="order_expiration_date" class="form-control" value="<?php echo (!empty($saved_data["order_expiration_date"]) && $saved_data["order_expiration_date"] !== '0000-00-00') ? htmlspecialchars($saved_data["order_expiration_date"]) : ""; ?>">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                    ...
                    <div class="form-section">
                        <h3 class="section-title"><i class="fa fa-clock-o"></i> Administration</h3>
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                    ...
                    <div class="form-section">
                        ...
                        <div id="secondary-medications-container">
                            <?php for ($i = 1; $i <= 4; $i++): 
                                // Show block if it's #1 or if it has existing data
                                $has_data = !empty($existing_secondary_medications[$i]['order_medication']);
                                $should_show = ($i == 1) || $has_data;
                                error_log("=== DEBUG LOADING: Block #$i - has_data: " . ($has_data ? 'true' : 'false') . ", should_show: " . ($should_show ? 'true' : 'false'));
                            ?>
                            <div class="secondary-medication-block" id="secondary-med-<?php echo $i; ?>" style="border: 1px solid #ddd; padding: 15px; margin-bottom: 15px; border-radius: 5px; <?php echo $should_show ? '' : 'display: none;'; ?>">
                                <div class="row">
                                    <div class="col-md-10">
                                ...
                                <div class="row">
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input type="text" name="secondary_medication_<?php echo $i; ?>" id="secondary_medication_<?php echo $i; ?>" class="form-control secondary-search-input" 
                                                       placeholder="Search inventory or type medication name" 
                                                       value="<?php echo htmlspecialchars($existing_secondary_medications[$i]['order_medication'] ?? ''); ?>"
                                                       data-secondary-index="<?php echo $i; ?>">
                                                <div class="input-group-append">
                                                    <button type="button" class="btn btn-outline-secondary" onclick="clearSecondaryInventory(<?php echo $i; ?>)" title="Clear selection">
                                                        <i class="fa fa-times"></i>
                                                    </button>
                                                </div>
                                            </div>
                                            <div id="secondary-suggestions-<?php echo $i; ?>" class="search-suggestions" style="display: none;"></div>
                                            <!-- Hidden fields for inventory selection -->
                                            <input type="hidden" name="secondary_inventory_drug_id_<?php echo $i; ?>" id="secondary-selected-drug-id-<?php echo $i; ?>" value="<?php echo htmlspecialchars($existing_secondary_medications[$i]['inventory_drug_id'] ?? ''); ?>">
                                            <input type="hidden" name="secondary_inventory_lot_number_<?php echo $i; ?>" id="secondary-selected-drug-lot-<?php echo $i; ?>" value="<?php echo htmlspecialchars($existing_secondary_medications[$i]['inventory_lot_number'] ?? ''); ?>">
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <input type="date" name="secondary_expiration_date_<?php echo $i; ?>" id="secondary_expiration_date_<?php echo $i; ?>" class="form-control" 
                                                   value="<?php echo (!empty($existing_secondary_medications[$i]['order_expiration_date']) && $existing_secondary_medications[$i]['order_expiration_date'] !== '0000-00-00') ? $existing_secondary_medications[$i]['order_expiration_date'] : ''; ?>">
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
...
if (document.getElementById("iv_access_date") && document.getElementById("iv_access_date").value) {
        ...
        // Multi-Secondary Medication Functions
        function addSecondaryMedication() {
            console.log('Adding secondary medication');
            
            // Find the next hidden secondary medication block
            let visibleCount = 0;
            for (let i = 1; i <= 4; i++) {
            ...
            console.log('Currently visible blocks:', visibleCount);
            
            if (visibleCount >= 4) {
                showNotification('Maximum of 4 secondary medications reached.', 'warning');
                return;
            }
            
            // Find the next hidden block
            for (let i = 2; i <= 4; i++) {
                const block = document.getElementById('secondary-med-' + i);
                if (block && block.style.display === 'none') {
                    block.style.display = 'block';
                    showNotification('Secondary medication #' + i + ' added. Please fill in the details.', 'success');
                    
                    // Initialize search functionality for this new block
                    initializeSecondarySearch(i);
        ...
        function clearSecondaryMedication(medicationNumber) {
            console.log('Clearing secondary medication #' + medicationNumber);
            
            if (medicationNumber === 1) {
                // For the first one, just clear the fields but don't hide it
                const inputs = document.querySelectorAll('#secondary-med-1 input, #secondary-med-1 select, #secondary-med-1 textarea');
                ...
                showNotification('Secondary medication #1 cleared.', 'info');
            } else {
                // For others, hide the block and clear fields
                const block = document.getElementById('secondary-med-' + medicationNumber);
                if (block) {
                    block.style.display = 'none';
                    
                    // Clear all fields in this block
                    ...
                    showNotification('Secondary medication #' + medicationNumber + ' removed.', 'info');
        ...
        // Secondary Medication Inventory Search Functions
        function initializeSecondarySearch(index) {
            console.log('=== DEBUG SEARCH: Initializing search for secondary medication #' + index);
            const searchInput = document.getElementById('secondary_medication_' + index);
            if (searchInput) {
                console.log('=== DEBUG SEARCH: Found search input for block #' + index);
                
                // Remove any existing event listeners to avoid duplicates
                searchInput.removeEventListener('input', searchInput._searchHandler);
                searchInput.removeEventListener('keydown', searchInput._keydownHandler);
                
                // Create new event handlers
                searchInput._searchHandler = function() {
                    console.log('=== DEBUG SEARCH: Input event on block #' + index + ', value:', this.value);
                    performSecondarySearch(this.value, index);
                };
                
                searchInput._keydownHandler = function(e) {
                    handleSecondaryKeydown(e, index);
                };
                
                searchInput.addEventListener('input', searchInput._searchHandler);
                searchInput.addEventListener('keydown', searchInput._keydownHandler);
                
                // Hide suggestions when clicking outside
                document.addEventListener('click', function(e) {
                    if (!e.target.closest('#secondary-med-' + index)) {
                        hideSecondarySuggestions(index);
                    }
                });
                
                console.log('=== DEBUG SEARCH: Search initialization complete for block #' + index);
            } else {
                console.error('=== DEBUG SEARCH: Could not find search input for block #' + index);
            }
        }
        
        function performSecondarySearch(query, index) {
            console.log('Performing secondary search for index ' + index + ':', query);
            ...
            fetch('/interface/modules/custom_modules/oe-module-inventory/get-drug-for-infusion.php?search=' + encodeURIComponent(query) + '&type=medication_name')
                .then(response => response.json())
                .then(data => {
                    console.log('Secondary search results for index ' + index + ':', data);
                    if (data.success && (data.drugs || data.results)) {
                        // Handle both response formats - newer API uses 'results', older uses 'drugs'
                        const medications = data.drugs || data.results;
                        displaySecondarySuggestions(medications, index);
                    } else {
                        hideSecondarySuggestions(index);
                    }
                })
                .catch(error => {
                    console.error('Secondary search error for index ' + index + ':', error);
                    hideSecondarySuggestions(index);
                });
        }
        
        function displaySecondarySuggestions(drugs, index) {
            const suggestionsContainer = document.getElementById('secondary-suggestions-' + index);
            ...
            drugs.forEach((drug, drugIndex) => {
                console.log('=== DEBUG SEARCH: Processing drug #' + drugIndex + ' for block #' + index + ':', drug);
                
                // Skip out-of-stock drugs - check various possible field names
                const quantity = drug.quantity || drug.available_quantity || drug.qty || 0;
                if (quantity <= 0) {
                    console.log('=== DEBUG SEARCH: Skipping out-of-stock drug:', drug.name || drug.drug_name);
                    return;
                }
                ...
                // Handle different possible field names from different API responses
                const drugName = drug.name || drug.drug_name || drug.medication_name || 'Unknown';
                const drugForm = drug.form || drug.dosage_form || 'N/A';
                const lotNumber = drug.lot_number || drug.lot || 'N/A';
                const quantityUnit = drug.quantity_unit || drug.unit || '';
                const expirationDate = drug.expiration_date || drug.exp_date || 'N/A';
                
                item.innerHTML = `
                    <strong>${drugName}</strong><br>
                    <small>${drugForm} | Lot: ${lotNumber} | Available: ${quantity} ${quantityUnit} | Exp: ${expirationDate}</small>
                `;
                item.addEventListener('click', () => selectSecondaryInventoryItem(drug, index));
        ...
        function selectSecondaryInventoryItem(drug, index) {
            console.log('Selected secondary drug for index ' + index + ':', drug);
            
            // Handle different possible field names from different API responses
            const drugName = drug.name || drug.drug_name || drug.medication_name || '';
            const doseValue = drug.dose || drug.strength || '';
            const unitValue = drug.unit || drug.dose_unit || '';
            const strengthValue = drug.size || drug.strength || drug.concentration || '';
            const lotNumber = drug.lot_number || drug.lot || '';
            const ndcValue = drug.ndc_11 || drug.ndc_10 || drug.ndc || '';
            const drugId = drug.drug_id || drug.id || '';
            const expirationDate = drug.expiration_date || drug.exp_date || '';
            ...
            // Use dose field for dose, size only for strength
            const combinedDose = doseValue && unitValue ? doseValue + ' ' + unitValue : doseValue || unitValue;
            document.getElementById('secondary_dose_' + index).value = combinedDose;
            
            document.getElementById('secondary_route_' + index).value = 'IV'; // Default route for infusion
            
            // Use size only for strength (concentration percentage)
            document.getElementById('secondary_strength_' + index).value = strengthValue;
            
            document.getElementById('secondary_lot_number_' + index).value = lotNumber;
            document.getElementById('secondary_ndc_' + index).value = ndcValue;
            
            // Format expiration date
            if (expirationDate) {
                const expirationInput = document.getElementById('secondary_expiration_date_' + index);
                if (expirationInput) {
                    expirationInput.value = oeFormatShortDate_js(expirationDate);
                }
            }
            
            // Set hidden fields for inventory tracking
            document.getElementById('secondary-selected-drug-id-' + index).value = drugId;
            document.getElementById('secondary-selected-drug-lot-' + index).value = lotNumber;
            
            // Hide suggestions
            hideSecondarySuggestions(index);
            
            showNotification('Secondary medication #' + index + ' populated from inventory: ' + drugName, 'success');
        }
        
        function clearSecondaryInventory(index) {
            // Clear all fields for this secondary medication
            document.getElementById('secondary_medication_' + index).value = '';
            document.getElementById('secondary_dose_' + index).value = '';
            document.getElementById('secondary_strength_' + index).value = '';
            document.getElementById('secondary_route_' + index).value = '';
            document.getElementById('secondary_lot_number_' + index).value = '';
            document.getElementById('secondary_expiration_date_' + index).value = '';
            document.getElementById('secondary_ndc_' + index).value = '';
            document.getElementById('secondary_frequency_' + index).value = '';
            
            // Clear hidden inventory fields
            document.getElementById('secondary-selected-drug-id-' + index).value = '';
            document.getElementById('secondary-selected-drug-lot-' + index).value = '';
            
            hideSecondarySuggestions(index);
            showNotification('Secondary medication #' + index + ' cleared.', 'info');
        }
        
        function hideSecondarySuggestions(index) {
            const suggestionsContainer = document.getElementById('secondary-suggestions-' + index);
        ...
        function handleSecondaryKeydown(e, index) {
            // Handle keyboard navigation for suggestions (similar to primary search)
            const suggestions = document.querySelectorAll('#secondary-suggestions-' + index + ' .suggestion-item');
            // Implementation would be similar to primary search keyboard handling
        }
        
        // Initialize secondary search for all visible medication blocks on page load
        document.addEventListener('DOMContentLoaded', function() {
            console.log('=== DEBUG JS: Initializing secondary search for visible blocks');
            
            for (let i = 1; i <= 4; i++) {
                const block = document.getElementById('secondary-med-' + i);
                if (block && block.style.display !== 'none') {
                    console.log('=== DEBUG JS: Initializing search for visible block #' + i);
                    initializeSecondarySearch(i);
                }
            }
            
            // Also update the add button visibility based on how many blocks are visible
            let visibleCount = 0;
            for (let i = 1; i <= 4; i++) {
                const block = document.getElementById('secondary-med-' + i);
                if (block && block.style.display !== 'none') {
            ...
            console.log('=== DEBUG JS: ' + visibleCount + ' blocks are visible on page load');
