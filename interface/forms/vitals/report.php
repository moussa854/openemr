...
function vitals_report($pid, $encounter, $cols, $id, $print = true)
{
    ...
    if ($data) {
        ...
        foreach ($data as $key => $value) {
            if (
                ...
                // $key == "date" || $value == "" ||
                $key == "last_updated" || $value == "" ||  // <-- Now skips last_updated instead
    ...
    if ($key == "date") {
    $formatted_date = date("m/d/Y g:i A", strtotime($value)); // Format the date
...
//    if ($key == "date") {
//        $formatted_date = date("m/d/Y g:i A", strtotime($value)); // Format the date
//        $vitals .= "<td><div class='font-weight-bold d-inline-block'>" . xlt("Date") . ": </div></td><td><div class='text' style='display:inline-block'>" . text($value) . "</div></td>";
//        $count++;
//        if ($count == $cols) {
//            $count = 0;
//            $vitals .= "</tr><tr>\n";
